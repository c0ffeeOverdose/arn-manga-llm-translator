# Vendored from TareHimself/AnimeMangaInpainting-torchscript (lama_ffc.py, MIT)
# which trims the generator path from advimman/lama (Apache-2.0). Only the
# inference generator is kept; used by scripts/export-inpaint-onnx.py to load
# the dreMaz/AnimeMangaInpainting lama_large_512px.ckpt (gen_state_dict).
"""Fast Fourier Convolution ResNet generator (LaMa "big-lama" architecture).

Vendored and trimmed from https://github.com/advimman/lama
(saicinpainting/training/modules/ffc.py), which is licensed Apache-2.0. Only the
generator path needed for inference is kept: discriminators, squeeze-excitation
and the learnable spatial transform wrapper have been removed. Behaviour of the
retained modules is unchanged.

`build_big_lama()` returns a generator whose `state_dict` matches
`dreMaz/AnimeMangaInpainting`'s `lama_large_512px.ckpt` (`gen_state_dict`) with
`strict=True`.

FFC: Fast Fourier Convolution, NeurIPS 2020 -- https://github.com/pkumivision/FFC
...
# --- big-lama configuration ------------------------------------------------

BIG_LAMA_KWARGS = dict(
    input_nc=4,
    output_nc=3,
    ngf=64,
    n_downsampling=3,
...
def build_big_lama() -> FFCResNetGenerator:
    """The generator matching lama_large_512px.ckpt (untrained weights)."""
    return FFCResNetGenerator(**BIG_LAMA_KWARGS)


def load_generator(ckpt_path: str, map_location="cpu") -> FFCResNetGenerator:
    """Build the generator and load `gen_state_dict` from the checkpoint."""
    ckpt = torch.load(ckpt_path, map_location=map_location, weights_only=False)
    state = ckpt["gen_state_dict"] if "gen_state_dict" in ckpt else ckpt
    model = build_big_lama()
